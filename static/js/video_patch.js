        ...
        if(s.name==sceneName){
            $.each(s.hotspot,function(j,h){
                if (typeof(h.type)!='undefined'&&h.type!=null&&h.type=='img') {
                  ...
                  console.log('1'+h.chromakey_color)
                  var is_use_chromakey = false
                  var chromakey_color = ''
                  if ('is_use_chromakey' in h) {
                    is_use_chromakey = h.is_use_chromakey
                  }
                  if ('chromakey_color' in h) {
                    is_use_chromakey = h.is_use_chromakey
                    chromakey_color.replace('#','0x')+'|0.2|0.2'
                  }
                  var is_use_chromakey = h
                  krpano.call('addvideo_tie_hotspot("'+h.videourl+'","'+h.name+'","'+h.hotspottitle+'","'+h.ath+'","'+h.atv+'","'+h.rx+'","'+h.ry+'","'+h.rz+'",0,0,true,"'+h.depth+'",'+h.autoplay+',"'+h.width+'","'+h.height+'","'+flag+'","'+h.posterurl+'","'+h.locationurl+'","'+is_use_chromakey+'","'+ chromakey_color+'")');
                }
            });
            return false;
        }
